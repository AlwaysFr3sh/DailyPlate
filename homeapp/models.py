...
class  recipe(models.Model):
    ...
    def rate(user, newRating, self):
        #if not shared
        if(not self.shared()): return False
        #if user has already rated this recipe
        if(ratingHistory.objects.filter(user=user, recipe=self).exists()): return False

        self.sharedDetails.updateRating(newRating)
        ratingHistory(ratingGiven=newRating, user=user, sharedRecipe=self.sharedDetails)
        return True#success
    ...
    def __str__(self):
        ...
        if(self.shared()): rtn += ", " + self.sharedDetails.__str__()
        return rtn
...
class ratingHistory(models.Model):
    ...
    def __str__(self):
        return "pk:" + str(self.pk) + ", user:" + self.user.username + ", recipe:" + self.recipe.getTitle()
